"""Fail-closed CFPB Seed v05.2 candidate builder.

The v05.2 builder preserves all earlier lineage. It requires accepted decision
record v04, audited fuzzy guards, and an unchanged proportional core plus a
separately labelled coverage supplement.
"""
...
def load_accepted_v04(
    decision_path: Path,
) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    record = json.loads(Path(decision_path).read_text(encoding="utf-8"))
    if record.get("record_version") != "v04":
        raise ValueError("Seed v05.2 requires finalized decision record v04")
    gate = record.get("build_gate", {})
    if gate.get("scope") != "build_seed_v052_candidate" or not gate.get("passed"):
        raise ValueError("Seed v05.2 build gate is not passed")
    if record.get("source_eda_version") != "v05.1":
        raise ValueError("Seed v05.2 requires EDA v05.1 governance")
    parsed = record.get("rules", {})
    if set(parsed) != REQUIRED_DECISION_AREAS:
        raise ValueError("Decision v04 rule areas are missing or unexpected")
    validate_v052_rules(parsed)
    return record, parsed


def validate_v052_rules(rules: dict[str, dict[str, Any]]) -> None:
    ...
    coverage = quotas.get("coverage_supplement", {})
    if coverage.get("strategy") != "one_per_uncovered_eligible_stratum":
        raise ValueError("Seed v05.2 requires two-stage coverage sampling")
    if int(coverage.get("core_size_unchanged", 0)) != int(quotas["primary_seed_size"]):
        raise ValueError("Coverage supplement cannot reduce the proportional core")
    if coverage.get("prevalence_eligible") is not False:
        raise ValueError("Coverage supplement cannot be prevalence eligible")
    pii = rules["pii"]
...
def cap_seed_candidates(
    ...
    working, exact_diagnostics = deterministic_group_cap(
        working, "canonical_text_sha256", exact_cap, random_seed, "seed_exact_cap"
    )
    accounting["after_exact_cap"] = _frame_counts(working)
    family_cap = int(rules["template_families"]["max_per_family"])
    working, family_diagnostics = deterministic_group_cap(
        working, "family_signature", family_cap, random_seed, "seed_family_cap"
    )
    accounting["after_family_cap"] = _frame_counts(working)
    fuzzy_rule = rules["fuzzy_duplicates"]
    pair_path = decision_record_path.parent / fuzzy_rule["adjudicated_pairs_path"]
    cluster_path = decision_record_path.parent / fuzzy_rule["cluster_review_path"]
    bridge_path = decision_record_path.parent / fuzzy_rule["bridge_review_path"]
    working, fuzzy_diagnostics = audited_fuzzy_group_cap(
        working,
        _read_csv(pair_path),
        _read_csv(cluster_path),
        _read_csv(bridge_path),
        cap=int(fuzzy_rule["max_per_confirmed_cluster"]),
        random_seed=random_seed,
        maximum_unapproved_component_size=int(
            fuzzy_rule["max_cluster_size_without_explicit_approval"]
        ),
    )
...
def _generation_view(
    ...
    view = pd.DataFrame(
        {
            ...
            "source_release": "CFPB Seed v05.2 candidate",
    ...
    if list(view.columns) != expected:
        raise ValueError("Generation view columns disagree with decision v04")
    return view


def build_seed_v052(config: SeedV052Config) -> dict[str, Path]:
    decision_record, rules = load_accepted_v04(config.decision_record_path)
    ...
    if sha256_file(eda_manifest_path) != decision_record["source_manifest_sha256"]:
        raise ValueError("Decision v04 does not match the frozen EDA manifest")
    eda_manifest = json.loads(eda_manifest_path.read_text(encoding="utf-8"))
    if eda_manifest.get("version") != "v05.1":
        raise ValueError("Seed v05.2 requires EDA v05.1")
    ...
    for filename in [
        ...
        rules["fuzzy_duplicates"]["cluster_review_path"],
        rules["fuzzy_duplicates"]["bridge_review_path"],
    ]:
        _resolve_governance_input(
            config.decision_record_path, decision_record, filename
        )
    ...
    primary, primary_allocation = proportional_stratified_sample_with_accounting(
        population_candidates,
        int(quotas["primary_seed_size"]),
        strata,
        config.random_seed,
        namespace="population_core",
    )
    if len(primary) != int(quotas["primary_seed_size"]):
        raise ValueError("Primary seed quota was not met")
    coverage_rule = quotas["coverage_supplement"]
    coverage, coverage_accounting = select_coverage_supplement(
        population_candidates,
        primary,
        list(coverage_rule["stratify_by"]),
        config.random_seed,
        maximum_rows=int(coverage_rule["maximum_rows"]),
    )
    ...
    primary["release_split"] = "population_core"
    primary["prevalence_eligible"] = False
    coverage["record_id"] = coverage["Complaint ID"].map(
        lambda value: _stable_record_id(prefix, value)
    )
    coverage["seed_id"] = coverage["record_id"]
    coverage["release_split"] = "coverage_supplement"
    coverage["prevalence_eligible"] = False
    ...
    enrichment["prevalence_eligible"] = False
    ...
    seed = pd.concat([primary, coverage, enrichment], ignore_index=True)
    ...
    population_release = seed.loc[seed["release_split"].eq("population_core")].copy()
    coverage_release = seed.loc[seed["release_split"].eq("coverage_supplement")].copy()
    enrichment_release = seed.loc[seed["release_split"].eq("enrichment")].copy()
    generation = _generation_view(seed, rules)

    config.output_dir.mkdir(parents=True, exist_ok=True)
    paths = {
        "seed_parquet": config.output_dir / "cfpb_seed_v052.parquet",
        "seed_jsonl": config.output_dir / "cfpb_seed_v052.jsonl",
        "population_core_parquet": config.output_dir / "cfpb_seed_v052_population_core.parquet",
        "coverage_supplement_parquet": config.output_dir / "cfpb_seed_v052_coverage_supplement.parquet",
        "enrichment_parquet": config.output_dir / "cfpb_seed_v052_enrichment.parquet",
        "stress_parquet": config.output_dir / "cfpb_seed_v052_stress_test.parquet",
        "excluded_parquet": config.output_dir / "cfpb_seed_v052_excluded.parquet",
        "generation_parquet": config.output_dir
        / "cfpb_seed_v052_generation_input.parquet",
        "generation_jsonl": config.output_dir / "cfpb_seed_v052_generation_input.jsonl",
        "primary_allocation_csv": config.output_dir / "cfpb_seed_v052_primary_allocation.csv",
        "coverage_accounting_csv": config.output_dir / "cfpb_seed_v052_coverage_accounting.csv",
    }
    seed.to_parquet(paths["seed_parquet"], index=False)
    seed.to_json(paths["seed_jsonl"], orient="records", lines=True, force_ascii=False)
    population_release.to_parquet(paths["population_core_parquet"], index=False)
    coverage_release.to_parquet(paths["coverage_supplement_parquet"], index=False)
    ...
    primary_allocation.to_csv(paths["primary_allocation_csv"], index=False, encoding="utf-8-sig")
    coverage_accounting.to_csv(paths["coverage_accounting_csv"], index=False, encoding="utf-8-sig")

    output_rows = {
        "seed_parquet": len(seed),
        "seed_jsonl": len(seed),
        "population_core_parquet": len(population_release),
        "coverage_supplement_parquet": len(coverage_release),
        ...
        "primary_allocation_csv": len(primary_allocation),
        "coverage_accounting_csv": len(coverage_accounting),
    }
    manifest = {
        "release": "CFPB Seed v05.2 candidate",
        "release_status": "privacy_qa_pending",
        "benchmark_eligible": False,
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "random_seed": config.random_seed,
        "parent_eda_manifest_sha256": sha256_file(eda_manifest_path),
        "parent_decision_record_v04_sha256": sha256_file(
            config.decision_record_path
        ),
        "governance_inputs": decision_record["governance_inputs"],
        "source_rows": len(corpus),
        "primary_rows": len(primary),
        "coverage_supplement_rows": len(coverage),
        ...
        "candidate_cap_accounting": cap_accounting,
        "two_stage_sampling": {
            "proportional_core_rows": len(primary),
            "coverage_supplement_rows": len(coverage),
            "coverage_supplement_prevalence_eligible": False,
            "raw_population_cells_reference": 814,
            "v051_core_covered_cells_reference": 456,
        },
        "rules": rules,
        "outputs": {
            name: {
    ...
    manifest_path = config.output_dir / "seed_v052_manifest.json"
